...
tgapiyenileme=input("Telegram Mikabot api ve secret'ın kayıtlısını kullanayım mı? (y/n)")
if tgapiyenileme=="y":
...
binanceapiyenileme=input("Binance api ve secret'ın kayıtlısını kullanayım mı? (y/n)")
if binanceapiyenileme=="y":
    binance_api=input("Binance Api Key giriniz: ")
    binance_secret=input("Binance Secret Key giriniz: ")
################################################## Değişkeler:
...
def trailinghesapla():
    if io1d[-1]>50:
        ...
        return trailingyuzde50altindayken #round((trailingyuzde / 4),2)
...
def threaded_buy_position(symbol, leverage, amount, liste):
    thread = threading.Thread(target=buy_position, args=(symbol, leverage, amount, liste))
    thread.start()

def threaded_sell_position(symbol, leverage, amount, liste):
    thread = threading.Thread(target=sell_position, args=(symbol, leverage, amount, liste))
    thread.start()

def threaded_close_position(symbol, liste):
    thread = threading.Thread(target=close_position, args=(symbol, liste))
    thread.start()
...
def almakkosulu():
    myresult = (io1d[-1]>50 and apalayimmi) or (io1d[-1]>48.9 and apalayimmi)# and (cift_ema_sinyal(io1d)[0]) and (not son_bes_esit_mi(io1d))) 
    return myresult
            ...
            if pos["Position"]:
                if pos["Position"]>0:
                    ...
                    if pos["P&L (%)"]>(yuzdekackazanincakapatsin) or pos["P&L (%)"]<(-1*trailinghesapla()):
                        ...
                        if groupacilanlar:
                            if not any(item[1] == binle(pos["Symbol"]) for item in groupacilanlar):
                    ...
                    if io1d[-1]<49:
                        if groupacilanlar:
                            ...
                            io49unaltinda.append(binle(pos["Symbol"]))
                    myp=pos["Mark Price"] #get_price(binle(pos["Symbol"]))
                    tprice.append(myp)
...
def AnaFonkKA(raw_text):
    ...
    if almakkosulu(): #result[0][0]>-1:# and io1d[-1]>50.1 and (extract_market_buying_power(mytextio[0])>1 or cift_ema_sinyal(iopower)[0]) and cift_ema_sinyal(io1d)[0]:#and cift_ema_sinyal(io1d)[0] and io1d[-1]!=49.9 and io1d[-1]!=50 and io1d[-1]!=50.1: # and not io15m1h4hdusuktemi():
        ...
        for coin in result:
            ...
            if binle(coin[0]) in mysymbols3 and coin[1]<1.6 and coin[1]>1 and coin[2]>1.025 and coin[3]<6.5: # and acabilirmiyim(binle(coin[0])):
        ...
        for coin in myFKAlist:
            if coin in mylonglarGenel:
                ...
                #mylonglarGenel.append(coin)
                buy_position(coin, myleverage, get_my_cost_hazir, "mylonglarGenel")
    ...
    if "Btc düşüş trendinde olduğu için," in raw_text:
        ...
        for coin in longacMA:
            if coin in mylonglarMA:
                ...
                #mylonglarMA.append(coin)
                buy_position(coin, myleverage, get_my_cost_hazir, "mylonglarMA")
    ...
    if cift_ema_sinyal(io1d)[1] and io1d[-1]!=49.9 and io1d[-1]!=50 and io1d[-1]!=50.1:#ortalamayagorepozisyonacayimmi("short"): #not is_above_last_period_average(io1d[-1],io1d,smaperiod): #not check_arrowsIO(mytextio[0]) and io1d[len(io1d)-1]<altustsinir[0]:
        for coin in shortAc:
            if coin in myshortlarCi:
                ...
                #myshortlarCi.append(coin)
                sell_position(coin, myleverage, get_my_cost_hazir, "myshortlarCi")
                print(f"{coin} short açıldı")
...
def AnaFonkIOF2(raw_text):
    ...
    longacilacaklar=[]
    kapatilacaklar=[]
    if almakkosulu():# io1d[-1]>48.9 and apalayimmi:
        for mylist in parsed_data:
            if binle(mylist[0]) in mysymbols3 and mylist[1]>2 and mylist[2]>0.4 and mylist[3]>1 and mylist[3]<1.65 and mylist[4][0] and mylist[4][1] and mylist[4][2] and mylist[4][3]:
        ...
        for coin in longAc:
            if coin in mylonglarIOF:
                ...
                if True:#check_arrowsIO(mytextio[0]):
                    #mylonglarIOF.append(coin)
                    buy_position(coin, myleverage, get_my_cost_hazir, "mylonglarIOF")
                    print(f"{coin} long açıldı")
        """
        for coin in mylonglarIOF:
            if coin in longAc:
    ...
    if io1d[-1]<49.9: # and (extract_market_buying_power(mytextio[0])<1):#not is_above_last_period_average(io1d[-1],io1d,smaperiod): # and not io15m1h4hyuksektemi(): #not check_arrowsIO(mytextio[0])  and io1d[len(io1d)-1]<altustsinir[0]: #len(combined_list2) > 0: 
        ...
        for coin in shortAc:
            if coin in myshortlarIOF:
                ...
                #myshortlarIOF.append(coin)
                sell_position(coin, myleverage, get_my_cost_hazir, "myshortlarIOF")
                print(f"{coin} short açıldı")
                #telegram_client.send_message(alert_user, f"{coin}'a SHORT posizyon açıldı. büyüklüğü: {myleverage}x kaldıraçlı, {mycost} USDT harcamalı, yani {myleverage * mycost} dolar büyüklüğünde.")
        """
        for coin in myshortlarIOF:
...
def AnaFonkGriov2(raw_text):
    ...
    for mylist in parsed_data:
        if almakkosulu() and mylist[1]>2 and mylist[2]>=1  and mylist[3]<1.9 and mylist[4][3]>49.8 and mylist[4][4]>49.8:
            print(mylist[0])
...
def AnaFonkIoGroup(raw_text):
    ...
    longacilacaklar=[]
    if almakkosulu():
        if io1d[-1]>49 and apalayimmi and parsed_data["group_data"]["12h"]>49.8 and parsed_data["group_data"]["1d"]>49.8 and parsed_data["group_data"]["short_term_buy_power"]>=1 and parsed_data["group_data"]["market_volume_share"]>=2:
            print("grup iyi durumda grio",parsed_data["group_data"]["group"])
            #print("merhaba")
            for coin in parsed_data["coin_data"]:
                if (coin["nakit"] * 0.01 * parsed_data["group_data"]["market_volume_share"]>=0.04) and coin["trend_up"][3] and coin["trend_up"][4] and coin["mts"]>=1 and coin["mts"]<1.7:
                    if binle(coin["coin"]) in mysymbols3:
        ...
        for coin in longacilacaklar:
            if coin in mylonglarGenel:
                ...
                #mylonglarGenel.append(coin)
                buy_position(coin, myleverage, get_my_cost_hazir * 1, "mylonglarGenel")
                print(f"Grio ile, {coin} long açıldı")

                if False: #not [parsed_data["group_data"]["group"], coin] in groupacilanlar:
...
def dusenler_satis():
    ...
    top_100_symbols = mysymbols3[:100] #symbols[:100]
    ...
    return [dropped_coins, dropped_coins1_5]

def dusenler_satis2():
    #client = Client(api_key='your_api_key', api_secret='your_api_secret')

    # USDT-M Futures piyasasında işlem gören coin'leri al
    exchange_info = binanceclient.futures_coin_exchange_info()
    symbols = [s['symbol'] for s in exchange_info['symbols'] if 'USDT' in s['baseAsset']]
    ...
    for symbol in top_100_symbols:
        klines = binanceclient.futures_klines(symbol=symbol, interval=Client.KLINE_INTERVAL_5MINUTE, limit=3)
        if len(klines) < 3:
            continue  # Yeterli veri yok

        # Açılış fiyatlarını al
        open_prices = [float(kline[1]) for kline in klines]
        if ((open_prices[0]-open_prices[-1])/open_prices[0])*100>1: #(open_prices[-1] / open_prices[0] - 1) * 100 < -1: 
            dropped_coins.append(symbol)
        if ((open_prices[0]-open_prices[-1])/open_prices[0])*100>1.5: #(open_prices[-1] / open_prices[0] - 1) * 100 < -1.5:
            dropped_coins1_5.append(symbol)
    print(f"düşenlersatış: {[dropped_coins,dropped_coins1_5]}")
    return [dropped_coins,dropped_coins1_5]
...
def AnaFonkSatis():
    ...
    if io1d[-1]<49:
        ...
        if len(mydusenler[0])>5:
            for coin in mydusenler[1]:
...
def AnaFonkSSR3(raw_text):
    ...
    print(f"ssr ile long açılacaklar: {longacilacaklar}")
    for coin in longacilacaklar:
        if coin in mylonglarGenel:
            print(f"{coin} zaten vardı SSR")
        elif coin in yasaklilist:
            print(f"Açılamayan coin: {coin}")
        else:
            #mylonglarGenel.append(coin)
...
async def main():
    ...
    async def handler(event):
        ...
        elif event.raw_text.startswith("Korelasyon Şiddeti Raporu"): #ci i d 5m
        ...
        elif event.raw_text.startswith("Akıllı Skoru En Fazla Olan Coinler"): #SSR
            #AnaFonkIOF2(event.raw_text)   
            while True:
                if calissinmi:
                    AnaFonkSSR3(event.raw_text)
                    print("ssr")
                    break
                else:
                    await asyncio.sleep(5)
        ...
        elif "L1 N.Payı:%" in event.raw_text and "LPOOL N.Payı:%" in event.raw_text:   #Grio   # event.raw_text.startswith("Belirtilen Coin Grubu İçin Nakit Girişi Raporu") and not "%100" in event.raw_text: 
            #AnaFonkIoGroup(event.raw_text)   
            while True:
                if calissinmi:
    ...
    while True:
        if True:
            ...
            while True:
                if calissinmi:
                    await telegram_client.send_message(target_user, "ssr" if io1d[-1]>48.9 else "io")
                    await asyncio.sleep(2*rastgele_sayi(30,45))  # 100 ile 400 saniye arasında rastgele bir saniyede bir mesaj gönder
                    break
                else:
                    await asyncio.sleep(2)
            ...
            while True:
                if calissinmi:
                    await telegram_client.send_message(target_user, "ssr" if io1d[-1]>48.9 else "io") #mysent[1] if kacinci == 0 else mysent[0])#'sdv')
                    await asyncio.sleep(2*rastgele_sayi(35,100))
                    break
                else:
                    await asyncio.sleep(2)
            ...
            await asyncio.sleep(2*rastgele_sayi(15,30))
