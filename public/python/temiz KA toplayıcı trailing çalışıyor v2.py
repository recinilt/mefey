...
trailingyuzde=2.5 #yüzde düşünce kapanır.
...
async def main():
    ...
    while True:
        if True:
            ...
            await telegram_client.send_message(target_user, kaci[0]) #mysent[rastgele_komut()] )#'marketanaliz')
            await asyncio.sleep(rastgele_sayi(35,100))
            await telegram_client.send_message(target_user, kaci[1]) #mysent[1] if kacinci == 0 else mysent[0])#'sdv')
