...
def AnaFonkGriov2(raw_text):
    ...
    for mylist in parsed_data:
        if almakkosulu() and mylist[1]>5 and mylist[2]>1.5 and mylist[3]>1 and mylist[3]<1.65 and mylist[4][3]>50 and mylist[4][4]>50:
            print(mylist[0])
...
def AnaFonkIoGroup(raw_text):
    ...
    longacilacaklar=[]
    if almakkosulu():
        if io1d[-1]>49 and apalayimmi and parsed_data["group_data"]["12h"]>50 and parsed_data["group_data"]["1d"]>50 and parsed_data["group_data"]["short_term_buy_power"]>=1.5 and parsed_data["group_data"]["market_volume_share"]>=4:
            print("grup iyi durumda grio")
            #print("merhaba")
            for coin in parsed_data["coin_data"]:
                if (coin["nakit"] * 0.01 * parsed_data["group_data"]["market_volume_share"]>=0.05) and coin["trend_up"][3] and coin["trend_up"][4] and coin["mts"]>1 and coin["mts"]<1.65:
                    if coin["coin"] in mysymbols3:
...
async def main():
    ...
    while True:
        if True:
            ...
            while True:
                if calissinmi:
                    await telegram_client.send_message(target_user, "iof") #mysent[rastgele_komut()] )#'marketanaliz')
                    await asyncio.sleep(rastgele_sayi(35,100))
                    break
                else:
                    await asyncio.sleep(2)
