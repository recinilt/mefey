...
async def async_buy_position(client, symbol, leverage, amount, liste):
    symbol = binle(symbol)
    try:
        ...
        price = float(await client.get_symbol_ticker(symbol=symbol.upper())['price'])
        quantity = round(amount * leverage / price, precision)
...
async def async_sell_position(client, symbol, leverage, amount, liste):
    symbol = binle(symbol)
    try:
        await client.futures_change_leverage(symbol=symbol, leverage=leverage)
        precision = get_symbol_precision(symbol)
...
async def async_close_position(client, symbol, liste):
    symbol = binle(symbol)
    try:
        ...
        for position in positions:
            if float(position['positionAmt']) != 0:
                ...
                if symbol in mylonglarGenel:
                    mylonglarGenel.remove(symbol)
        eklesil(symbol, liste, "sil")

    except Exception as e:
        print(f"Error: {e}")

async def buy_multiple_coins(client, coins, leverage, amount, takiplistesi):
    tasks = [async_buy_position(client, coin, leverage, amount, takiplistesi) for coin in coins]
    await asyncio.gather(*tasks)

async def sell_multiple_coins(client, coins, leverage, amount, liste):
    tasks = [async_sell_position(client, coin, leverage, amount, liste) for coin in coins]
    await asyncio.gather(*tasks)

async def close_multiple_coins(client, coins, takiplistesi):
    tasks = [async_close_position(client, coin, takiplistesi) for coin in coins]
    await asyncio.gather(*tasks)

def ornekasyncfonk():
    coins = ['BTCUSDT', 'ETHUSDT', 'BNBUSDT']  # Örnek coin listesi
    asyncio.run(buy_multiple_coins(coins))
...
def dusenler_satis():
    ...
    return [dropped_coins, dropped_coins1_5, increased_coins,increased_coins1_5,top_100_symbols]
...
def AnaFonkSatis():
    ...
    if len(mydusenler[0])>10:
        for coin in mydusenler[4]:
            shortacilacaklar=[]
            if not coin in myshortlarGenel:
                shortacilacaklar.append(coin)
                #sell_position(coin, 20, 1, "myshortlarGenel")
                threaded_sell_position(coin, 20, 1, "mylonglarGenel")
            #asyncio.run(sell_multiple_coins(binanceclient,shortacilacaklar,22,1,"myshortlarGenel"))
    elif len(mydusenler[0])<10:
        ...
        if not len(myshortlarGenel)==0:
            ...
            #asyncio.run(close_multiple_coins(binanceclient,myshortlarGenel,"myshortlarGenel"))
            for coin in myshortlarGenel:
    ...
    #Alış:
    if True:
        if len(mydusenler[2])>10:
            for coin in mydusenler[4]:
                longacilacaklar=[]
                if not coin in mylonglarGenel:
                    longacilacaklar.append(coin)
                    #buy_position(coin, 20, 1, "mylonglarGenel")
                    threaded_buy_position(coin, 20, 1, "mylonglarGenel")
                #asyncio.run(buy_multiple_coins(binanceclient,longacilacaklar,22,1,"mylonglarGenel"))
        elif len(mydusenler[2])<10:
            ...
            if not len(mylonglarGenel)==0:
                ...
                #    kapatılacaklar.append(coin)
                    #close_position(coin,"mylonglarGenel")
                #asyncio.run(close_multiple_coins(binanceclient,mylonglarGenel,"mylonglarGenel"))
                #mylonglarGenel.clear()
                #myshortlarGenel.clear()
                for mycoin in mylonglarGenel:
                    threaded_close_position(mycoin,"mylonglarGenel")
